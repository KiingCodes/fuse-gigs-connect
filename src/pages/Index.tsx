      ...
      <section className="gradient-hero px-4 py-20 text-center md:py-28">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="container mx-auto max-w-3xl">
          ...
          <p className="mb-8 text-lg text-primary-foreground/70 md:text-xl">
            Connect with talented small businesses and freelancers in your community.
          </p>
          <div className="mx-auto flex max-w-xl gap-2">
            <div className="relative flex-1">
              <Search className="absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-muted-foreground" />
              <Input placeholder="Search hustles..." value={search} onChange={(e) => setSearch(e.target.value)} className="h-12 pl-10 bg-background text-foreground" />
            </div>
            <Link to="/explore">
              <Button className="h-12 gradient-primary text-primary-foreground px-6 gap-2">Explore <ArrowRight className="h-4 w-4" /></Button>
            </Link>
          </div>
        </motion.div>
      </section>
        ...
        <section className="py-12">
          <div className="container mx-auto px-4">
            <div className="mb-8 flex items-center justify-between">
              ...
              <Link to="/explore"><Button variant="outline" size="sm" className="gap-1">View All <ArrowRight className="h-4 w-4" /></Button></Link>
            </div>
        ...
        <section className="bg-muted/50 py-12">
          <div className="container mx-auto px-4">
            ...
            <div className="flex flex-wrap gap-2">
              <Badge variant={!selectedCategory ? "default" : "outline"} className={`cursor-pointer px-4 py-2 text-sm transition-all ${!selectedCategory ? "gradient-primary text-primary-foreground border-0" : ""}`} onClick={() => setSelectedCategory(undefined)}>All</Badge>
              {categories.map((cat) => (
        ...
        <div className="container mx-auto px-4">
          ...
          {isLoading ? (
            ...
            <div className="py-16 text-center">
              ...
              {user && <Link to="/create"><Button className="mt-4 gradient-primary text-primary-foreground">Post Your Hustle</Button></Link>}
            </div>
          )}
        </div>
      </section>
              ...
              <CardContent className="flex flex-col items-center gap-4 p-8 sm:flex-row sm:justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-primary-foreground/80">Get more views and appear at the top. Starting from R19.</p>
                  </div>
                </div>
                <Link to="/boost"><Button variant="secondary" size="lg" className="font-semibold gap-1 whitespace-nowrap"><TrendingUp className="h-4 w-4" /> Boost Now</Button></Link>
              </CardContent>
            </Card>
          </div>
        </section>
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            <Card className="overflow-hidden border-0 bg-gradient-to-br from-violet-600 to-indigo-700 shadow-elevated">
              <CardContent className="flex flex-col items-center gap-4 p-8 sm:flex-row sm:justify-between">
                ...
                <Link to="/academy"><Button variant="secondary" size="lg" className="font-semibold gap-1 whitespace-nowrap">Start Learning</Button></Link>
              </CardContent>
            </Card>
              ...
              <CardContent className="flex flex-col items-center gap-4 p-8 sm:flex-row sm:justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-white/80">Ask questions, share services & connect</p>
                  </div>
                </div>
                <Link to="/community"><Button variant="secondary" size="lg" className="font-semibold gap-1 whitespace-nowrap">Join Chat</Button></Link>
              </CardContent>
            </Card>
            <Card className="overflow-hidden border-0 bg-gradient-to-br from-orange-600 to-amber-700 shadow-elevated">
              <CardContent className="flex flex-col items-center gap-4 p-8 sm:flex-row sm:justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-white">Sell Products 🛍️</h3>
                    <p className="text-sm text-white/80">Sell materials, bundle with your services</p>
                  </div>
                </div>
                <Link to="/products"><Button variant="secondary" size="lg" className="font-semibold gap-1 whitespace-nowrap">Start Selling</Button></Link>
              </CardContent>
            </Card>
            {user && (
      ...
      {!user && (
        <section className="gradient-primary py-16 text-center">
          <div className="container mx-auto px-4">
            ...
            <Link to="/auth"><Button size="lg" variant="secondary" className="font-semibold">Get Started Free</Button></Link>
          </div>
        </section>
      )}
